...
- [NAT Sharing / Device-as-Router Abuse Guard Contract](../contracts/vpn/NAT_SHARING_GUARD_CONTRACT.md)
...
## 12. NAT Sharing / Device-as-Router Guard

NodeAgent 后续需要按 NAT Sharing Guard 契约实现 aggregate per-session counters、
redacted risk events 和可选 enforcement hooks。NodeAgent 不得记录 raw destination
IP/domain/URL、DNS 历史或 packet payload，也不得把风险事件标签设计成高基数字段。
