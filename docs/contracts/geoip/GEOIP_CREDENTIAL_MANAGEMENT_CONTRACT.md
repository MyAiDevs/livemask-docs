...
## 1.1 与 Admin Job Center 的关系

GeoIP source credential 配置仍属于 `/admin/geoip/sources`，因为它管理的是
第三方账号、endpoint、license key、API token 等敏感配置。

但 GeoIP 的 `Trigger Update`、定时更新、重试、取消、运行历史和事件日志应迁移到
...
| 能力 | 所属页面 |
| --- | --- |
| 配置 MaxMind / DB-IP / IP2Location / Hackl0us 账号和 token | `/admin/geoip/sources` |
| Verify source credential | `/admin/geoip/sources/{source}` 可作为快捷入口，底层应产生 `geoip_source_verify` job |
| Trigger GeoIP update | `/admin/jobs?job_type=geoip_source_update` |
| GeoIP update schedule | `/admin/jobs/schedules` |
| GeoIP run history / events | `/admin/jobs/runs?job_type=geoip_source_update` |

Job Center 不得收集或展示 credential 明文，只能引用 source credential 状态。
