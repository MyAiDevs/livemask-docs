...
| `main` | Remote pre-release branch | Staging / pre-release smoke on self-hosted runner | Merge `dev` into `main` |
...
```text
dev is stable across required repos
  -> reviewed merge dev -> main
  -> child repo push on main dispatches staging-promote to livemask-ci-cd
  -> livemask-ci-cd runs staging smoke
...
| `staging-promote` | child repo push to `main` | `livemask-ci-cd` staging smoke | Pre-release validation |
