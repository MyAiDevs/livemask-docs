...
| CI/CD OpenAPI Drift Check | [TASK-CICD-OPENAPI-DRIFT-CHECK-001](tasks/TASK-CICD-OPENAPI-DRIFT-CHECK-001.md) | `livemask-ci-cd` | Ready | Add CI/CD integration for Backend `scripts/validate-openapi.sh` so Backend API/OpenAPI drift is enforced beyond backend-local validation. | Can run in parallel with Admin Swagger UI; assign to `livemask-ci-cd` Cursor window. |
