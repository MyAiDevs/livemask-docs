...
| Backend API Documentation | [TASK-BACKEND-SWAGGER-API-DOCS-001](tasks/TASK-BACKEND-SWAGGER-API-DOCS-001.md) | `livemask-backend` | Ready | New requirement: add complete OpenAPI 3 / Swagger documentation, machine-readable API JSON, Swagger UI, auth/RBAC/error/schema coverage, sensitive-example redaction checks, and validation for future iteration. Handoff: [BACKEND-SWAGGER-API-DOCS-CURSOR_HANDOFF.md](cursor-handoffs/BACKEND-SWAGGER-API-DOCS-CURSOR_HANDOFF.md). | Can run in parallel with reconnect work if backend lease does not collide; completion must merge to backend `dev`, rerun validation on `dev`, and push `origin/dev`. |
