...
- NAT Sharing Guard（TASK-DOC-NAT-SHARING-GUARD-001）— ✅ Contract completed. 防止客户端设备被当作 NAT/路由器共享 VPN 的隐私保护风控契约。结论：可做 best-effort 检测/限流/吊销，但不能宣称 NodeAgent 单点 100% 阻止 rooted/admin 设备或外部路由器。后续实现任务已拆分为 Backend、NodeAgent、App、Admin、CI/CD 五个 ready TASK。
...
| Docs Task State Ledger | [TASK-DOCS-TASK-STATE-LEDGER-001](tasks/TASK-DOCS-TASK-STATE-LEDGER-001.md) | `livemask-docs` | ✅ Completed | Added `docs/development/task-state-ledger.json` as the machine-readable module/task snapshot and `scripts/check-task-state-ledger.py` to validate task IDs, repos, statuses, task docs, SHA shape, dependencies, and completed-task validation evidence. Wired into `bash scripts/check-docs.sh`. | Keep ledger updated when accepting completion reports; expand coverage as modules close. |
| AI Project Status Onboarding | [TASK-DOCS-AI-PROJECT-STATUS-ONBOARDING-001](tasks/TASK-DOCS-AI-PROJECT-STATUS-ONBOARDING-001.md) | `livemask-docs` | ✅ Completed | Added `docs/development/AI_PROJECT_STATUS_ONBOARDING.md` as the first-read project status file for any AI tool. It explains current phase, source-of-truth files, completed governance capabilities, known gaps, high-priority next tasks, report processing, task intake, async health checks, and anti-error rules. | Keep this file updated whenever project phase or highest-priority next tasks change. |
| Codex Dispatcher Role | [TASK-DOCS-CODEX-DISPATCHER-ROLE-001](tasks/TASK-DOCS-CODEX-DISPATCHER-ROLE-001.md) | `livemask-docs` | ✅ Completed | Added `docs/development/CODEX_TASK_DISPATCHER_ROLE.md` as the source of truth for Codex's role as the docs-side task dispatch center. It defines report intake, evidence review, docs/Issue sync, module summary, next Cursor dispatch, async audit, and prohibited behaviors. Strengthened the task-state ledger check so a module cannot be marked `completed` while it still contains open child tasks. | Other AI editors should read the role doc before relying on Codex/docs dispatcher behavior. |
| Docs Cursor Brief Template | [TASK-DOCS-CURSOR-BRIEF-TEMPLATE-001](tasks/TASK-DOCS-CURSOR-BRIEF-TEMPLATE-001.md) | `livemask-docs` | ✅ Completed | Added `docs/development/CURSOR_TASK_BRIEF_TEMPLATE.md` so every new Cursor dispatch includes target repo, branch, must-read docs, scope, out-of-scope areas, expected files, validation, dev merge requirement, docs handoff, and next unlock conditions. | Use this template for all new Cursor task briefs. |
| Task Reconciler | [TASK-CICD-TASK-RECONCILER-001](tasks/TASK-CICD-TASK-RECONCILER-001.md) | `livemask-docs` / `livemask-ci-cd` | ✅ Completed | First lightweight reconciler validates the task state ledger locally and in docs checks. It intentionally avoids auto-closing Issues; the first automation goal is drift detection. | Strict cross-repo Issue sync and active lease registry are now completed; next automation step is close/reopen guard. |
| Strict Issue Sync | [TASK-CICD-ISSUE-SYNC-STRICT-001](tasks/TASK-CICD-ISSUE-SYNC-STRICT-001.md) | `livemask-ci-cd` | ✅ Completed | Task branch `a96a742`, dev merge and remote `origin/dev` `bb262a3`. Added Docker-independent `scripts/issue-sync-strict.sh` to search docs + runtime repo Issues by TASK ID, fail on missing/ambiguous states with structured exit codes, support text/json output, require token, write `GITHUB_OUTPUT`, and run verbose diagnostics. Added `.github/workflows/issue-sync-strict.yml` plus staging-smoke pre-Docker Issue sync strict check with `issues: read`. Validation PASS: `bash -n scripts/issue-sync-strict.sh`, `bash -n scripts/*.sh`, `git diff --check`, dev-merge-guard. | Next: guarded Issue close/reopen automation remains separate in `TASK-CICD-ISSUE-CLOSE-GUARD-001`; lease registry can build on strict scan behavior. |
| Issue Close Guard | [TASK-CICD-ISSUE-CLOSE-GUARD-001](tasks/TASK-CICD-ISSUE-CLOSE-GUARD-001.md) | `livemask-ci-cd` / `livemask-docs` | ✅ Completed | Task branch `8ce0d5e`, dev merge and remote `origin/dev` `41c7bd3`. Added `scripts/issue-close-guard.sh`, `.github/workflows/issue-close-guard.yml`, and staging-smoke dry-run evaluations for completed and ready/blocked tasks. Guard defaults to dry-run, refuses Epic close, blocks ambiguous Issue matches, records blocker reasons, and supports explicit `--write`. Validation PASS: `bash -n scripts/*.sh`, `git diff --check`. | No existing docs/ci-cd Issue was found to close during docs sync; remote governance audit is now completed separately. |
| Remote Governance Audit | [TASK-DOCS-GOVERNANCE-REMOTE-AUDIT-001](tasks/TASK-DOCS-GOVERNANCE-REMOTE-AUDIT-001.md) | `livemask-docs` | ✅ Completed | Extended `scripts/audit-task-center.py` with opt-in `--remote-issues`, `--remote-actions`, `--remote-refs`, `--remote-all`, `--remote-strict`, and `--remote-ref-self-test`. Base docs checks remain offline; remote drift is warning-only unless strict mode is requested. Validation PASS: offline audit, remote ref self-test, remote refs warning audit, remote Actions no-token warning audit, remote Issues JSON parse, docs check, and `git diff --check`. | Child repo AI/Cursor rule sync is now completed. |
| Active Cursor Lease Registry | [TASK-DOCS-LEASE-REGISTRY-001](tasks/TASK-DOCS-LEASE-REGISTRY-001.md) | `livemask-docs` / all repos | ✅ Completed | Added file-backed `docs/development/task-leases.json`, deterministic `scripts/check-task-leases.py`, docs check wiring, collision self-test, and lease start/end rules in the Cursor brief template and governance docs. | Lease rules are now mirrored into child repo `.cursorrules`; continue product/runtime dispatch. |
| Child Repo AI Rule Sync | [TASK-DOCS-CHILD-REPO-AI-RULE-SYNC-001](tasks/TASK-DOCS-CHILD-REPO-AI-RULE-SYNC-001.md) | `livemask-docs` / all runtime repos | ✅ Completed | Mirrored lease registry, completion report, docs boundary, remote audit, and Issue close guard rules into child repo `.cursorrules`. Child repo task/dev refs: backend `e9071cd` -> `4aa7116`, nodeagent `b5591f4` -> `0c4ed4f`, app `a384d7c` -> `18ed56c`, admin `0ab3f7d` -> `668cda6`, website `494beb0` -> `1a35282`, job-service `2ecb50e` -> `4f7ae82`, ci-cd `68c5a7f` -> `350ee0c`. | Governance-control-plane is complete for active dispatch; next priority returns to product/runtime reconnect tasks. |
| Backend API Documentation | [TASK-BACKEND-SWAGGER-API-DOCS-001](tasks/TASK-BACKEND-SWAGGER-API-DOCS-001.md) | `livemask-backend` | ✅ Completed | Task branch commits `f42cfc1`, `3d0c307`, `8e5e968`; dev merge and remote `origin/dev` `9de2f14`. Added OpenAPI 3.0.3 YAML with 124 documented paths, generated JSON, `/openapi.yaml`, `/openapi.json`, CORS for Admin, no public `/swagger/` route, embeddable Swagger UI HTML template, and `scripts/validate-openapi.sh` with 22 checks for spec validity, route coverage, secret scanning, JSON consistency, no public Swagger UI, Go build/vet/test, and whitespace. | Admin Swagger UI and CI/CD OpenAPI drift integration are now unblocked. Every later Backend API add/change/delete must update OpenAPI in the same task. |
| Admin Swagger UI | [TASK-ADMIN-SWAGGER-API-DOCS-UI-001](tasks/TASK-ADMIN-SWAGGER-API-DOCS-UI-001.md) | `livemask-admin` | ✅ Completed | Task branch commit `df1af3a`, dev merge and remote `origin/dev` `656d4d9`. Added `/admin/api-docs` using `swagger-ui-react`, protected by Admin `AuthGuard`, same-origin `/admin/openapi.json` and `/admin/openapi.yaml` rewrites, bearer-token injection through `requestInterceptor`, API Docs sidebar entry, loading/error/unavailable states, and tests for auth/error behavior. Validation PASS: `npx vitest run` 217/217, `npx next build` 56/56 pages, `git diff --check`, and dev validation rerun. | Follow-up needed for stale dev-server/build cache diagnostics after a browser reported 404 while curl returned HTTP 200. |
| Admin API Docs Cache Diagnostics | [TASK-ADMIN-API-DOCS-CACHE-INVALIDATION-001](tasks/TASK-ADMIN-API-DOCS-CACHE-INVALIDATION-001.md) | `livemask-admin` | ✅ Completed | Task branch commit `2c2a627`, dev merge and remote `origin/dev` `a4231cb`. Added safe `/admin/api/build-info`, no-store OpenAPI fetch, build ref badge, distinct unauthorized/proxy-unreachable/unavailable states, diagnostics panel, and tests. Validation PASS: `npx vitest run` 225/225, `npx next build` 56 pages, `git diff --check`. Curl: `/admin/api-docs` 200, `/admin/api/build-info` 200, `/admin/openapi.json` 404, Backend `/openapi.json` 404. | Root cause is Backend runtime route missing, not stale Admin build. Dispatch `TASK-BACKEND-OPENAPI-RUNTIME-ROUTE-FIX-001`. |
| Backend OpenAPI Runtime Route Fix | [TASK-BACKEND-OPENAPI-RUNTIME-ROUTE-FIX-001](tasks/TASK-BACKEND-OPENAPI-RUNTIME-ROUTE-FIX-001.md) | `livemask-backend` | Ready | Follow-up from Admin diagnostics: Backend runtime at `localhost:8080` returns 404 for `/openapi.json`, causing Admin `/admin/openapi.json` proxy to return 404. | Dispatch to `livemask-backend`; fix runtime `/openapi.json` and `/openapi.yaml` while keeping public Swagger UI unavailable. |
| CI/CD OpenAPI Drift Check | [TASK-CICD-OPENAPI-DRIFT-CHECK-001](tasks/TASK-CICD-OPENAPI-DRIFT-CHECK-001.md) | `livemask-ci-cd` | ✅ Completed | Task branch `task/TASK-CICD-OPENAPI-DRIFT-RECONNECT-SMOKE-001` commit `10f271b`, dev merge and remote `origin/dev` `c5f628a`. Added `scripts/openapi-drift-smoke.sh`, staging-smoke integration, and PASS/SKIP/FAIL classification for Backend `scripts/validate-openapi.sh`. Validation PASS: `bash -n` for openapi drift/protocol endpoint/smoke scripts, `git diff --check`, and dev-merge-guard validation. | Backend API documentation module is complete; future Backend API tasks must keep OpenAPI aligned. |
...
| App Android Functional Validation | [TASK-APP-ANDROID-FUNCTIONAL-VALIDATION-001](tasks/TASK-APP-ANDROID-FUNCTIONAL-VALIDATION-001.md) | `livemask-app` | ✅ Completed (Android-primary) | AppClient feature validation completed under Android-first policy. Dev merge `374f6d7`, remote `origin/dev` `374f6d7`. Validation PASS: `flutter analyze`, `flutter test` 567 tests, Android APK build, and emulator run. iOS remains deferred platform hardening; macOS arm64/x64, Windows, Linux, and Web were not part of this task's acceptance scope. | Continue Android-first App feature backlog; track iOS/platform hardening separately. |
| App Runtime Closed-Loop Validation | [TASK-DOCS-APP-RUNTIME-CLOSED-LOOP-VALIDATION-001](tasks/TASK-DOCS-APP-RUNTIME-CLOSED-LOOP-VALIDATION-001.md) | `livemask-docs` / `livemask-app` | ✅ Completed | App development docs now require runtime launch, debug-log evidence, Backend API communication, and task-relevant NodeAgent / Job Service communication before App feature tasks can be considered complete. Added dev-only role preset login policy for normal user, promotion ambassador, sponsor ambassador, trial/expired user, and optional debug operator validation. | Follow-up in `livemask-app`: update `.cursorrules` and implement dev-only preset login switcher. |
...
| Backend Protocol Stability | [TASK-BACKEND-PROTOCOL-STABILITY-001](tasks/TASK-BACKEND-PROTOCOL-STABILITY-001.md) | `livemask-backend` | ✅ Completed | Added Backend executor handlers for protocol endpoint rollout/rollback waves and NodeAgent release rollout/rollback waves, App reconnect hint API, connect capability gating, assignment state queries, active reconnect hint queries, LKG confirmation, and route registration. Validation PASS: `go build ./...`, `go test ./...` across 23 packages with 22 protocol tests and 10 nodeagent tests. | NodeAgent assignment apply/LKG/event retry, App reconnect consumption, Admin UI, and CI/CD SKIP-to-PASS runtime smoke remain follow-ups. |
| Backend Protocol LKG / Rollback API | [TASK-BACKEND-PROTOCOL-LKG-ROLLBACK-API-001](tasks/TASK-BACKEND-PROTOCOL-LKG-ROLLBACK-API-001.md) | `livemask-backend` | ✅ Completed | Task branch `22c2f40`, dev merge and remote `origin/dev` `9a06111`. Added real Admin LKG/rollback fields for protocol templates, template eligibility, assignment list, and assignment detail. LKG comes from `node_assignment_states.lkg_config_hash`, `lkg_template_version`, and `lkg_healthy_at`; no-LKG returns explicit null/none/false state. Added assignment list/detail routes and 24 tests for LKG present/absent, rollback availability, rolled-back assignment, serialization, and sensitive config leak prevention. Validation PASS: `go test ./...` (20/20 packages), `go vet ./...`, `go build ./...`, `git diff --check`. | Admin can remove mock LKG fallback; CI/CD should add protocol assignment LKG/rollback smoke assertions. |
| NodeAgent Protocol Stability | [TASK-NODEAGENT-PROTOCOL-STABILITY-001](tasks/TASK-NODEAGENT-PROTOCOL-STABILITY-001.md) | `livemask-nodeagent` | ✅ Completed | Task branch `76e0849`, integration validation `c99fd5e`, dev merge and remote `origin/dev` `0fa3e9c`. Replaced the legacy ProtocolEndpointManager assignment path with a stability subsystem that consumes Backend assignment/event/capability endpoints, renders and validates candidate Singbox config, persists LKG, rolls back on apply or health-check failure, retries events with exponential backoff, reports capabilities, exposes `/protocol/*` local endpoints, and emits `livemask_nodeagent_protocol_event_queue_depth`. Validation PASS: `go build ./cmd/nodeagent/`, `go vet ./cmd/nodeagent/ ./internal/assign/`, `go test ./...` (10/10 packages). | Runtime smoke should rerun with Backend and NodeAgent both on `dev` so assignment pull, event reporting, rollback, and capability reporting become PASS. |
...
| Job Service Protocol Stability | [TASK-JOBS-PROTOCOL-STABILITY-001](tasks/TASK-JOBS-PROTOCOL-STABILITY-001.md) | `livemask-job-service` | ✅ Completed | Task branch `694b824`, dev merge `16d9ba0`, remote `origin/dev` `16d9ba0`. Added paused run state, pause/resume service and HTTP routes, pause-aware protocol rollout/rollback executors, NodeAgent release rollout/rollback executors with wave splitting, per-node locks, retry/backoff, cancel/pause awareness, failure thresholds, dry-run, and secret-leak tests. Validation PASS: `go test ./... -count=1`, `go vet ./...`, `go build ./cmd/job-service/`. | Backend must implement `/internal/job-executors/nodeagent-release/*` and `/internal/job-executors/protocol-endpoint/*` executor APIs; CI/CD should add rollout/rollback smoke. |
| CI/CD Protocol Stability | [TASK-CICD-PROTOCOL-STABILITY-001](tasks/TASK-CICD-PROTOCOL-STABILITY-001.md) + [TASK-CICD-PROTOCOL-LKG-ROLLBACK-SMOKE-001](tasks/TASK-CICD-PROTOCOL-LKG-ROLLBACK-SMOKE-001.md) | `livemask-ci-cd` | ✅ Completed | Base task branch `dccfbd6`, dev merge `d721677`; LKG smoke task branch `1a5a009`, dev merge and remote `origin/dev` `c7842e8`. Enhanced `scripts/protocol-endpoint-smoke.sh` with NodeAgent `endpoint_ready` HMAC event, assignment DB checks, reconnect hint endpoint probes, NodeAgent `rolled_back` HMAC event, and LKG/rollback assertions for template list/detail, eligibility, and assignment list/detail. Enhanced `scripts/protocol-capability-smoke.sh` with LKG/rollback assertions and extended secret leak scans. Syntax validation PASS: protocol endpoint smoke, protocol capability smoke, aggregate smoke, and `git diff --check`. | Runtime smoke should be rerun against Docker dev-local with Backend/NodeAgent/App on current `dev` so remaining SKIP paths become PASS. |
| Admin Protocol Stability | [TASK-ADMIN-PROTOCOL-STABILITY-001](tasks/TASK-ADMIN-PROTOCOL-STABILITY-001.md) + [TASK-ADMIN-PROTOCOL-LKG-MOCK-RETIRE-001](tasks/TASK-ADMIN-PROTOCOL-LKG-MOCK-RETIRE-001.md) | `livemask-admin` | ✅ Completed | Original task branch `08f1b3f`, dev merge `823f4fe`; LKG/rollback UI task commit `c988b63`, dev merge `986dc9c`; mock-retire task commit `0ab0e4c`, dev merge and remote `origin/dev` `4b46435`. Added protocol template/assignment create pages, create/update API client actions, filtered list query params, LKG status cards, rollback availability display, rollback event timelines, LKG-highlighted version history, and strict real-data tests. Mock fallback is isolated to 404/501/network read failures; 401/403 and mutations never fallback. Validation PASS: latest `npx vitest run` 207/207, `npx next build`, `git diff --check`. | Admin protocol LKG mock-retire is closed; continue runtime smoke and App reconnect real-backend closure. |
| Backend Reconnect Runtime API | [TASK-BACKEND-RECONNECT-HINT-RUNTIME-001](tasks/TASK-BACKEND-RECONNECT-HINT-RUNTIME-001.md) | `livemask-backend` | ✅ Completed | Task commit `4249450`, dev merge and remote `origin/dev` `1442e64`. Added App-auth `GET /api/v1/connect/config?session_id=...` and `GET /api/v1/reconnect-hints`, safe reconnect hint envelope, session-owned connect config re-fetch, and tests for auth/method/missing params, nil safety, empty hints, response whitelist, and secret leak prevention. Validation PASS: connect/handler/protocol tests, `go test ./...` (18 packages), `go vet ./...`, `go build ./...`, `git diff --check`. | App can un-mock reconnect runtime; CI/CD can add reconnect hint smoke. |
| App Reconnect Stability | [TASK-APP-RECONNECT-STABILITY-001](tasks/TASK-APP-RECONNECT-STABILITY-001.md) | `livemask-app` | ✅ Completed | Original reconnect flow task branch `6eefee4`, dev merge `17e83c9`; latest polling increment dev merge and remote `origin/dev` `5a433f9`. Added `fetchConnectConfig()` to real/mock connect API clients, refactored reconnect into a safe three-step flow, added `ReconnectHintsResponse` / `PolledReconnectHint`, implemented `fetchReconnectHints({sessionId})` for real/mock clients, and added reconnect hint polling, deduplication, start/stop lifecycle, and non-disruptive error handling. Validation PASS: latest `flutter test` 592/592; `flutter analyze` has pre-existing info/warnings only with 0 new issues from this task. | Backend runtime API is now available at `1442e64`; App should run real Backend cutover task and CI/CD should add reconnect hint runtime smoke. |
| App Reconnect Real Backend Cutover | [TASK-APP-RECONNECT-RUNTIME-REAL-BACKEND-001](tasks/TASK-APP-RECONNECT-RUNTIME-REAL-BACKEND-001.md) | `livemask-app` | ✅ Completed | Task branch commit `09e69c9`, dev merge and remote `origin/dev` `e797875`. `fetchConnectConfig()` now accepts optional `sessionId`; real client sends `session_id` only when non-empty; reconnect flows pass `state.sessionId`; mock reconnect data remains isolated to `AUTH_CLIENT_MODE=mock`. Validation PASS: `flutter analyze --no-fatal-warnings --no-fatal-infos`, `flutter test` 597/597, `git diff --check`, and dev-merge-guard on integration/dev. | CI/CD reconnect hint runtime smoke is now unblocked. |
| CI/CD Reconnect Hint Runtime Smoke | [TASK-CICD-RECONNECT-HINT-RUNTIME-SMOKE-001](tasks/TASK-CICD-RECONNECT-HINT-RUNTIME-SMOKE-001.md) | `livemask-ci-cd` | ✅ Completed | Task branch `task/TASK-CICD-OPENAPI-DRIFT-RECONNECT-SMOKE-001` commit `10f271b`, dev merge and remote `origin/dev` `c5f628a`. Enhanced `scripts/protocol-endpoint-smoke.sh` `[11c]` with connect session creation, `GET /api/v1/connect/config?session_id=...`, `GET /api/v1/reconnect-hints?session_id=...`, safe-field assertions, forbidden internal-field checks, and no-session loose-scope check. Validation PASS: `bash -n` scripts, `git diff --check`, and dev-merge-guard validation. | Protocol stability module is complete in repo-native ledger; future staging smoke may still report SKIP when runtime seed/session data is unavailable. |
...
2. AppClient validation policy: Android is now the primary App feature
   ...
   `TASK-DOCS-APP-ANDROID-FIRST-VALIDATION-001`. Current Android functional
   evidence is recorded by `TASK-APP-ANDROID-FUNCTIONAL-VALIDATION-001` at
   App dev merge `374f6d7`.
   Build-only evidence is insufficient: new App tasks must provide runtime
   debug logs and Backend/NodeAgent/Job Service closed-loop evidence per
...
5. `TASK-BACKEND-PROTOCOL-STABILITY-001` is completed. Continue with
   `TASK-NODEAGENT-PROTOCOL-STABILITY-001`,
   CI/CD protocol stability SKIP-to-PASS runtime verification. `TASK-ADMIN-PROTOCOL-STABILITY-001`
   is completed at Admin dev merge `823f4fe`; `TASK-APP-RECONNECT-STABILITY-001`
   is completed at App dev merge `17e83c9`.
