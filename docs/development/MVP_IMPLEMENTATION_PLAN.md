...
| Admin API Docs Cache Diagnostics | [TASK-ADMIN-API-DOCS-CACHE-INVALIDATION-001](tasks/TASK-ADMIN-API-DOCS-CACHE-INVALIDATION-001.md) | `livemask-admin` | Ready | Root-cause task for `/admin/api-docs` stale 404 confusion. Requires no-store/cache-busting for API Docs route/OpenAPI fetch, safe Admin build/ref diagnostics, explicit Backend OpenAPI proxy failure states, and curl evidence distinguishing route existence from `/admin/openapi.json` proxy availability. | Dispatch to `livemask-admin` Cursor window on branch `task/TASK-ADMIN-API-DOCS-CACHE-INVALIDATION-001`. |
