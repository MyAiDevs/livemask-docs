...
| CI/CD Protocol Stability | [TASK-CICD-PROTOCOL-STABILITY-001](tasks/TASK-CICD-PROTOCOL-STABILITY-001.md) | `livemask-ci-cd` | ✅ Completed | Task branch `dccfbd6`, dev merge `d721677`, remote `origin/dev` `d721677`. Enhanced `scripts/protocol-endpoint-smoke.sh` with NodeAgent `endpoint_ready` HMAC event, assignment DB checks, reconnect hint endpoint probes, and NodeAgent `rolled_back` HMAC event. Syntax validation PASS: protocol endpoint smoke, protocol capability smoke, aggregate smoke, and `git diff --check`. New Backend-dependent sections report SKIP until deployed. | Backend must deploy protocol assignment/event/reconnect endpoints; NodeAgent/App reconnect tasks must provide real runtime signals for SKIP-to-PASS closure. |
| Admin Protocol Stability | [TASK-ADMIN-PROTOCOL-STABILITY-001](tasks/TASK-ADMIN-PROTOCOL-STABILITY-001.md) | `livemask-admin` | ✅ Completed | Original task branch `08f1b3f`, dev merge `823f4fe`; latest LKG/rollback incremental task commit `c988b63`, dev merge and remote `origin/dev` `986dc9c`. Added protocol template/assignment create pages, create/update API client actions, filtered list query params, unified hooks, 30s node capability refetch, Node Detail hook update, template/assignment LKG status cards, rollback availability display, rollback event timeline icons/badges, LKG-highlighted version history, and rollback dialog LKG recommendation. Validation PASS: `npx vitest run` 168 tests, `npx next build` (55 routes). | Backend should return LKG/rollback fields in protocol template/detail and assignment/detail APIs; runtime smoke remains required for real data closure. |
