...
| Admin Swagger UI | [TASK-ADMIN-SWAGGER-API-DOCS-UI-001](tasks/TASK-ADMIN-SWAGGER-API-DOCS-UI-001.md) | `livemask-admin` | ✅ Completed | Task branch commit `df1af3a`, dev merge and remote `origin/dev` `656d4d9`. Added `/admin/api-docs` using `swagger-ui-react`, protected by Admin `AuthGuard`, same-origin `/admin/openapi.json` and `/admin/openapi.yaml` rewrites, bearer-token injection through `requestInterceptor`, API Docs sidebar entry, loading/error/unavailable states, and tests for auth/error behavior. Validation PASS: `npx vitest run` 217/217, `npx next build` 56/56 pages, `git diff --check`, and dev validation rerun. | Backend API documentation surface is complete; CI/CD OpenAPI drift integration remains. |
