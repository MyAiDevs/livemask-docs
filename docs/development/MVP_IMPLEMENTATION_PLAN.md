...
| App Reconnect Stability | [TASK-APP-RECONNECT-STABILITY-001](tasks/TASK-APP-RECONNECT-STABILITY-001.md) | `livemask-app` | ✅ Completed | Task branch `6eefee4`, dev merge `17e83c9`, remote `17e83c9`. Added `fetchConnectConfig()` to real/mock connect API clients and refactored reconnect into a safe three-step flow: fetch fresh config, restore pre-hint state for unsupported/skeleton/engine-pending configs, and only create a new session/start tunnel when the new config is ready. Focused tests PASS: reconnect hint, connect models, mock connect API, real connect API. | Backend must provide `GET /api/v1/connect/config`; CI/CD should add reconnect hint runtime smoke. |
