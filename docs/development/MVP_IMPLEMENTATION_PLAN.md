...
| Task Reconciler | [TASK-CICD-TASK-RECONCILER-001](tasks/TASK-CICD-TASK-RECONCILER-001.md) | `livemask-docs` / `livemask-ci-cd` | ✅ Completed | First lightweight reconciler validates the task state ledger locally and in docs checks. It intentionally avoids auto-closing Issues; the first automation goal is drift detection. | Next: strict cross-repo Issue sync and active lease registry. |
...
| Active Cursor Lease Registry | [TASK-DOCS-LEASE-REGISTRY-001](tasks/TASK-DOCS-LEASE-REGISTRY-001.md) | `livemask-docs` / all repos | Ready | Follow-up task to track active Cursor leases and detect overlapping repo/file ownership before edits. | Implement after strict Issue sync or in parallel if collisions increase. |
