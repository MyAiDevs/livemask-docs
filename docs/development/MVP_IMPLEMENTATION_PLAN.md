...
| Backend API Documentation | [TASK-BACKEND-SWAGGER-API-DOCS-001](tasks/TASK-BACKEND-SWAGGER-API-DOCS-001.md) | `livemask-backend` / `livemask-admin` | Ready | New requirement: all Backend APIs must align with OpenAPI/Swagger. Add complete OpenAPI 3 documentation, machine-readable OpenAPI JSON for Admin/CI, Admin-authenticated Swagger UI only inside logged-in `livemask-admin`, auth/RBAC/error/schema coverage, sensitive-example redaction checks, and route/API drift validation. Handoff: [BACKEND-SWAGGER-API-DOCS-CURSOR_HANDOFF.md](cursor-handoffs/BACKEND-SWAGGER-API-DOCS-CURSOR_HANDOFF.md). | Every later Backend API add/change/delete must update OpenAPI in the same task; completion must prove Backend does not expose public unauthenticated Swagger UI and Admin login can view Swagger UI. |
| Admin Swagger UI | [TASK-ADMIN-SWAGGER-API-DOCS-UI-001](tasks/TASK-ADMIN-SWAGGER-API-DOCS-UI-001.md) | `livemask-admin` | Blocked | Admin must provide the human-facing Swagger UI after Backend exposes validated OpenAPI JSON. The UI must require Admin login and must fail closed when OpenAPI JSON is unavailable. | Blocked by `TASK-BACKEND-SWAGGER-API-DOCS-001`; do not start until Backend OpenAPI JSON route and validation evidence land. |
