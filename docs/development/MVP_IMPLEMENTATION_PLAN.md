...
| CI/CD Governance | [TASK-CICD-WORKSPACE-PATH-MIGRATION-001](tasks/TASK-CICD-WORKSPACE-PATH-MIGRATION-001.md) | `livemask-ci-cd` / all repos | ✅ Ready | Defines migration from `/Users/sammytan/Documents/New project 2` to `/Users/sammytan/Developer/LiveMask`, old-path stop rules for Cursor/Codex, `LIVEMASK_WORKSPACE_ROOT` script discovery expectations, and validation commands for guard/local runtime/App iOS builds. | Implement after all active windows are clean; reopen Cursor windows from the new path only. |
