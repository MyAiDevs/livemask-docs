...
| Admin Protocol Stability | [TASK-ADMIN-PROTOCOL-STABILITY-001](tasks/TASK-ADMIN-PROTOCOL-STABILITY-001.md) | `livemask-admin` | ✅ Completed | Task branch `08f1b3f`, dev merge `823f4fe`, remote `origin/dev` `823f4fe`. Added protocol template/assignment create pages, create/update API client actions, filtered list query params, unified protocol template hooks, 30s node capability refetch, and Node Detail hook update. Validation PASS: `npx vitest run` 168 tests, `npx next build`. | Backend API deployment and protocol stability runtime smoke remain required for real data closure. |
