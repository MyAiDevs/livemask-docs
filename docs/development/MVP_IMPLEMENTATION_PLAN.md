...
| Admin Swagger UI | [TASK-ADMIN-SWAGGER-API-DOCS-UI-001](tasks/TASK-ADMIN-SWAGGER-API-DOCS-UI-001.md) | `livemask-admin` | Ready | Admin must provide the human-facing Swagger UI using Backend `/openapi.json` or `/openapi.yaml`. The UI must require Admin login and must fail closed when OpenAPI JSON is unavailable. | Unblocked by Backend dev `9de2f14`; assign to `livemask-admin` Cursor window. |
