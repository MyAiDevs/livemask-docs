...
| CI/CD Admin Control Plane | [TASK-CICD-ADMIN-CONTROL-PLANE-SMOKE-001](tasks/TASK-CICD-ADMIN-CONTROL-PLANE-SMOKE-001.md) | `livemask-ci-cd` | ✅ Completed (syntax + wiring; runtime blocked) | Script discovery confirmed `scripts/admin-control-plane-smoke.sh` is missing, then enhanced existing domain scripts instead: `system-settings-smoke.sh`, `jobs-smoke.sh`, `protocol-capability-smoke.sh`, `release-control-smoke.sh`, and `sentry-config-smoke.sh`. `scripts/smoke.sh` and staging workflow were unchanged because enhanced scripts were already wired. Validation PASS: `bash -n` for touched scripts and `scripts/smoke.sh`, `git diff --check`, dev-merge-guard. Runtime smoke is blocked/SKIP because local dev runtime was not running. | Run enhanced smoke against a running dev-local/staging runtime; then update from syntax+wiring to runtime PASS. |
